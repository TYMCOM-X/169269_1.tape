...
BOOLEAN       Initialised,        \ Parameters set up ;
	...
	Time!test,		\ when true types out time of Simin outputs
				    with output line;
...
INTEGER ARRAY Timerblock[0:1],
		...
		Prompt!Script	  [0:Script!Limit],
		Prompt!Line	  [0:Port!Limit],
...
INTEGER       Max!port,           \ max ports this run;
		clock!flag1!count,
		clock!tot!count,
	      Timer!Count,	   \ count of timer interrupts mod 3;
	      Clock!Set,	  \ last time the clock was set;
...
FORWARD PROCEDURE DAY!CMD;
FORWARD STRING PROCEDURE hms(INTEGER Sec);
FORWARD PROCEDURE Clock!off;
...
BEGIN "SENDID"
    ...
    Prompt!Line[Line] _ Prompt!Script[Script];	\ resets prompt to initial prompt;
    ...
    IF Time!test THEN s!print(%"(" & hms(CALLI(0,'23)) & ")"%);
...
BEGIN "INITIALIZEFILES"
    ...
    INTEGER Chan,fileno,Texttab,Brchar;
...
\ bail;
		...
		BEGIN "not!end"
                   IF
		    NOT (Value _ EQU(Dataline[1 to 2],"#!"))
                   THEN
                   BEGIN "valid!text"
			IF
			    First
			THEN
				...
				Firstline[Fileno] _ Rectxt;
	                        Sdata:Sstring[rectxt] _ Dataline;
	                        Sdata:Sptr[rectxt]_Null!record;
                   	    END "first"
			ELSE
			    BEGIN "not!first"
				Nxttxt_NEW!RECORD(Sdata);
				Sdata:Sptr[Rectxt]_Nxttxt;
				Sdata:Sstring[Nxttxt]_Dataline;
...
BEGIN "INTERRUPTHANDLER"
    ...
    INTEGER M,Char,Temp,LINE,K;
	STRING st1,st2;
	CLOCK!TOT!COUNT _ CLOCK!TOT!COUNT + 1;

	IF
	    NOT CLOCK!FLAG1
	THEN
	begin
	    clock!flag1!count _ clock!flag1!count + 1;
	end;
	IF Debug!trace
	THEN
	BEGIN "trace"
	...
	THEN
		...
		Rate!Cmd("FULL");
	     END "reset";
    FOR K _ 1 STEP 1 UNTIL Max!line
    DO
    BEGIN "look"
	Channel _ Channel#[K];
...
\ pg              IF
 ...
 pg ;
     ...
     [8] BEGIN "d!conn"                         \ 8 SW!DISCONNECT ;
              ...
              THEN
              Disconnect(Line);
              Post#[Line]_Now+((Logon!delay[User!script[Line]]));
              User!switch[Line] _ Sw!logon;
         END "d!conn";
...
[7]      t!print( %"SUPERVISOR ERROR code ",CRLF%);
	ELSE
		t!print(%"UNKNOWN SUP ERROR, ERROR=",CVS(RESULT)%)
    END;
...
["4"]              BEGIN     \ END OF LOOP;
			...
			Post#[Line] _ now + ((Logon!Delay[Script] ));
                        User!prompt[Line] _ Sw!nothing;
...
["8"]              BEGIN  \ start at the top of script file;
                   ...
                   BEGIN
                        ...
                        Post#[Line]_Now+((Logon!delay[Script]));
                        User!switch[Line]_Sw!next!line;
                        IF
                             Debug!Trace
                        THEN
...
["P"]		BEGIN	\ Change the Prmpt;
			Prompt!Line[Line] _ LOP(DataLine) & NULL;
			User!switch[Line] _ Sw!Next!Line;
		END;
...
BEGIN "HELPCMD"
	...
	THEN
		Object _ "1";
    I _ CVD(Object);
	T!PRINT(%"PORT!LIMIT=",PORT!LIMIT,CRLF%);
    IF
         0 < I < Port!Limit
    THEN
         IF
...
END;
    ...
    BEGIN
         ...
         ELSE
              Printit(Line);
    END;
	t!print(%"ratio interrupt calls =",cvs(clock!flag1!count*1000/clock!tot!count),crlf%);
    IF
...
BEGIN "INITIALIZECMD"
    INTEGER Value,Script,width,digits,SCRIPT!ID;
    STRING Instring;
	...
	ELSE
		Cntrollee!quit _ Instring & CR & CR;
...
\    IF
    ...
    BEGIN
	 t!print(%crlf%);
	SCRIPT!ID _ SCRIPT + 1;
         Instring _ NULL;
         t!print( %"SCript ",SCRIPT!ID,",Enter Script file, Default is TESTF: "%);
         ...
         Instring _ Null;
         t!print( %"Script ",SCRIPT!ID,",Enter ID prefix, Default is ""."": "%);
         Instring_ Upper(Getdata);
         IF
              Instring = NULL
         THEN
         ...
         t!print( %"Script ",SCRIPT!ID,",Enter No of Users, Default is 1: "%);
         Instring _ Upper(Getdata);
         IF
              Instring = NULL
         THEN
    ...
    t!print(%"Script ",SCRIPT!ID,",Enter Prompt, default is ':' :"%);
	Instring _ Upper(Getdata);
	IF
		Instring = NULL
	THEN
		Prompt!Script[Script] _ ":"
	ELSE
		Prompt!Script[Script] _ Instring;
    Prompt!Line[Max!Port] _ Prompt!Script[Script];
         t!print( %"Script ",SCRIPT!ID,",Enter Logon delay, units - (seconds):"%);
         Instring _ Upper(Getdata);
         IF
              Instring = NULL
         THEN
         ...
         t!print( %"Script ",SCRIPT!ID,",Enter No of Loops, default is 1: "%);
         Instring _ Upper(Getdata);
         IF
              Instring = NULL
         THEN
...
REQUIRE 0 POLLING!INTERVAL;
	...
	THEN
	BEGIN "in!bounds"
	    FOR I _ (Line - 1) STEP 1 UNTIL Max!Line
	    DO
	    BEGIN "count"
		S!Print(%"**SCRIPT FILE  LINE ",I+1,"**",CRLF%);
		Recptr _ FirstLine[I];
		K _ 0;
		WHILE
...
\	prompt;
	OUT(o!chan,Prompt!Script[Script] & CRLF);
\	Logon delay;
...
**********************************************************************;
    ...
    BEGIN "DAYCMD"
	T!Print(%STDATE(CALLI(0,'14),0),tab%);
	T!Print(%hms(CALLI(0,'23)),crlf%);
    END "DAYCMD";

COMMENT****************************************************************
*                                                                     *
...
BEGIN "hms"
	...
	THEN
	    BEGIN
		TIME _ CVS(sec div 3600000) & ":";
		SETFORMAT(-2,digits);
	    END
	ELSE
	    time _ "   ";
	temp _ sec MOD 3600000;
	IF
	    sec > 60000
	THEN
	    BEGIN
		time _ time & CVS(temp DIV 60000) & ":";
		SETFORMAT(-2,digits);
	    END
	ELSE
	    time _ time & "   ";
	time _ time & CVS((temp _ temp MOD 60000) DIV 1000);
	SETFORMAT(-3,digits);
	time _ time & "." & CVS(temp MOD 1000);
	SETFORMAT(width,digits);
	RETURN(time);
